    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-8">
      ...
      <div className="flex justify-between items-center bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            <h1 className="text-lg sm:text-xl font-bold text-slate-900">Daily Check-In</h1>
            <p className="text-xs sm:text-sm text-slate-500 font-medium">Saturday, March 14</p>
          </div>
        </div>
        <div className="bg-teal-50 text-teal-700 px-3 py-1.5 rounded-full text-[10px] sm:text-xs font-bold flex items-center gap-1 border border-teal-100">
          <span>✓</span> <span className="hidden sm:inline">Submitted today</span>
        </div>
      </div>

      {/* 2. SLEEP CATEGORY */}
      <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm">
        <h2 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-6">Sleep</h2>
        <CustomSlider 
          label="Sleep Duration" value={sleepDuration} onChange={setSleepDuration} 
          min={0} max={12} step={0.5} formatValue={formatHours}
          leftLabel="0h" rightLabel="12h" 
        />
        <CustomSlider 
          label="Sleep Quality" value={sleepQuality} onChange={setSleepQuality} 
          min={1} max={5} step={1} formatValue={formatQuality}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm">
        <h2 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-6">Academics</h2>
        <CustomSlider 
          label="Study Hours Today" value={studyHours} onChange={setStudyHours} 
          min={0} max={10} step={0.5} formatValue={formatHours}
          leftLabel="0h" rightLabel="10h" 
        />
        <CustomSlider 
          label="Academic Workload" value={academicWorkload} onChange={setAcademicWorkload} 
          min={1} max={5} step={1} formatValue={formatWorkload}
          leftLabel="Very Light" rightLabel="Very Heavy" 
        />
      </div>

      {/* 4. ACTIVITY & SOCIAL CATEGORY */}
      <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm">
        <h2 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-6">Activity & Social</h2>
        <CustomSlider 
          label="Physical Activity" value={physicalActivity} onChange={setPhysicalActivity} 
          min={0} max={5} step={0.5} formatValue={formatHours}
          leftLabel="None" rightLabel="5h+" 
        />
        <CustomSlider 
          label="Social Interaction Time" value={socialInteraction} onChange={setSocialInteraction} 
          min={1} max={5} step={1} formatValue={formatSocial}
          leftLabel="Isolated" rightLabel="Very Social" 
        />
        <CustomSlider 
          label="Social Media Usage" value={socialMedia} onChange={setSocialMedia} 
          min={0} max={12} step={0.5} formatValue={formatHours}
          leftLabel="0h" rightLabel="12h+" 
        />
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm">
        <h2 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-6">Mental State</h2>
        <CustomSlider 
          label="Mood Level" value={moodLevel} onChange={setMoodLevel} 
          min={1} max={10} step={1} formatValue={formatOutof10}
          leftLabel="Very Low" rightLabel="Excellent" 
        />
        <CustomSlider 
          label="Stress Level" value={stressLevel} onChange={setStressLevel} 
          min={1} max={10} step={1} formatValue={formatOutof10}
          leftLabel="Calm" rightLabel="Very Stressed" 
        />
        <CustomSlider 
          label="Anxiety Level" value={anxietyLevel} onChange={setAnxietyLevel} 
          min={1} max={10} step={1} formatValue={formatOutof10}
          leftLabel="Relaxed" rightLabel="High Anxiety" 
        />
      </div>
      ...
      <div className="bg-teal-50 border border-teal-100 rounded-2xl p-4 sm:p-5 flex gap-3 items-start shadow-sm">
        ...
        <div>
          <h3 className="text-xs sm:text-sm font-bold text-teal-900">AI Processing</h3>
          <p className="text-[10px] sm:text-xs text-teal-700 mt-1 sm:mt-1.5 leading-relaxed">
            Your responses are analyzed by our Deep Neural Network (DNN) for immediate risk classification, and Temporal Fusion Transformer (TFT) for future forecasting with SHAP explainability.
          </p>
        </div>
      </div>
